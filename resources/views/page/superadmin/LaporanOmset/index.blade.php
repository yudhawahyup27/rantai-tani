                ...
                <table class="table mt-4">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge bg-{{ $rev->status === 'Lunas' ? 'success' : 'secondary' }}">
                                    {{ $rev->status ?? '-' }}
                                </span>
                            </td>
                            <td>
                                <form action="{{ route('admin.laporan-keuangan.update', $rev->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <select name="status" onchange="this.form.submit()" class="form-select form-select-sm">
                                        <option value="Pending" {{ $rev->status == 'Pending' ? 'selected' : '' }}>Pending</option>
                                        <option value="Lunas" {{ $rev->status == 'Lunas' ? 'selected' : '' }}>Lunas</option>
                                        <option value="Batal" {{ $rev->status == 'Batal' ? 'selected' : '' }}>Batal</option>
                                    </select>
                                </form>
                            </td>
                        </tr>
